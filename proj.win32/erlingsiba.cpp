    ...
    int       nCmdShow)
{
    ...
    return ErlingsibaPlayer::createAndRun();
}

int ErlingsibaPlayer::createAndRun(void)
{
    ErlingsibaPlayer *host = ErlingsibaPlayer::sharedInstance();
    int ret = host->run();
    ErlingsibaPlayer::purgeSharedInstance();
    return ret;
}

ErlingsibaPlayer *ErlingsibaPlayer::s_sharedInstance = NULL;

ErlingsibaPlayer *ErlingsibaPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new ErlingsibaPlayer();
    }
    return s_sharedInstance;
}

void ErlingsibaPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
ErlingsibaPlayer::ErlingsibaPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int ErlingsibaPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "erlingsiba-player");
...
void ErlingsibaPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void ErlingsibaPlayer::createViewMenu(void)
{
...
void ErlingsibaPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void ErlingsibaPlayer::updateWindowTitle(void)
{
}

void ErlingsibaPlayer::relaunch(void)
{
...
void ErlingsibaPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void ErlingsibaPlayer::onFileOpenProject(void)
{
...
void ErlingsibaPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"ErlingsibaPlayer");
...
void ErlingsibaPlayer::onFileProjectConfig(void)
{
...
void ErlingsibaPlayer::onFileRelaunch(void)
{
    relaunch();
}

void ErlingsibaPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void ErlingsibaPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void ErlingsibaPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void ErlingsibaPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void ErlingsibaPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT ErlingsibaPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    ErlingsibaPlayer *host = ErlingsibaPlayer::sharedInstance();
...
INT_PTR CALLBACK ErlingsibaPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
